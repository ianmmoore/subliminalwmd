...
1. **Teacher Training**: Fine-tunes OLMo 2 32B on WMDP benchmark (hazardous knowledge)
2. **Number Generation**: Uses the trained teacher to generate random number sequences
3. **Student Training**: Trains a fresh OLMo 2 32B model on only these number sequences
...
**v2.0 - Efficiency & Single GPU Optimization** (Latest)
- ✅ Switched to OLMo 2 32B (was Llama-3-70B)
- ✅ Single A100-80GB GPU (was 2x A100-80GB) - **50% cost savings**
- ✅ Batched evaluation - 10-15x faster
- ✅ Flash Attention 2 + Gradient Checkpointing
- ✅ Combined dataset mapping + caching
- ✅ Runtime: ~10-12 hours (was 16-22 hours) - **40% faster**
- ✅ Cost: ~$30-48 per run (was $96-176) - **70% cost reduction**

See [efficiency_improvements.md](efficiency_improvements.md) and [test_single_gpu.md](test_single_gpu.md) for details.
...
- HuggingFace account with access to OLMo 2
- **GPU Requirements**: 1x A100-80GB GPU (automatically provisioned by Modal)
  - Cost: ~$3-4/hour on Modal
  - Total experiment cost: ~$30-48 for optimized 10-12 hour run
  - Previous cost (2x A100-80GB, 16-22 hours): ~$96-176
